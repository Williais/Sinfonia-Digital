...
import { agendaService, Aviso, Evento } from "../../services/agenda.service";
...
export default function HomeScreen() {
  ...
  async function carregarTudo() {
    try {
      ...
      notificationService
        .registerForPushNotificationsAsync(authUser.id)
        .catch(console.error);

      const perfil = await profileService.getUserProfile();
      setUser(perfil);

      const [eventos, avisos, musicas, estatisticas, rankData] =
        await Promise.all([
  ...
  const renderHeroEvent = (evento: Evento) => {
    const hora = new Date(evento.date).toLocaleTimeString("pt-BR", {
      hour: "2-digit",
      minute: "2-digit",
    });
    return (
      ...
      <View style={{ marginTop: 16 }}>
        ...
        >
          <View style={styles.secondDateBox}>
            ...
            <Text style={styles.secondDateMonth}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.secondSubtitle}>
              {evento.type.toUpperCase()} • {evento.location}
            </Text>
          </View>
  ...
  const canManageAttendance =
    user?.role === "admin" || user?.role === "maestro" || user?.is_spalla;
    ...
    >
      ...
      <View style={styles.header}>
        <View>
          ...
          <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
            <Text style={styles.subGreeting}>
              {user?.instrument || "Instrumentista"}
            </Text>
            {user?.is_spalla && (
              <View style={styles.roleBadge}>
                <Text style={styles.roleText}>SPALLA</Text>
              </View>
            )}
            {user?.is_section_leader && !user?.is_spalla && (
              <View style={styles.roleBadge}>
                <Text style={styles.roleText}>CHEFE DE NAIPE</Text>
              </View>
            )}
        ...
        <TouchableOpacity onPress={() => router.push("/(tabs)/perfil")}>
          <Image
            source={{
              uri: user?.avatar_url || "https://github.com/shadcn.png",
            }}
            style={styles.avatar}
          />
        </TouchableOpacity>
        ...
        {proximosEventos.length > 0 ? (
          <>
            ...
            {proximosEventos.length > 1 &&
              renderSecondEvent(proximosEventos[1])}
          </>
        ) : (
      ...
      {musicaRecente ? (
        <TouchableOpacity
          ...
          onPress={() => router.push(`/acervo/${musicaRecente.id}`)}
        >
      ...
      <View style={styles.rankingCard}>
        {ranking.map((item, index) => (
          <View
            ...
            style={[
              styles.rankingRow,
              index !== ranking.length - 1 && styles.borderBottom,
            ]}
          >
