...
export default function AcervoListaScreen() {
  ...
  function verificarCategoria(musica: Musica, cat: string): boolean {
    ...
    const instrumentos = Object.keys(musica.partiturasPaths).map((i) =>
      i.toLowerCase(),
    );
    if (cat === "cordas")
      return instrumentos.some(
        (i) =>
          ...
          i.includes("cello") ||
          i.includes("baixo") ||
          i.includes("contrabaixo"),
      );
    if (cat === "sopros")
      return instrumentos.some(
        (i) =>
          i.includes("flauta") ||
          i.includes("clarinete") ||
          i.includes("oboé") ||
          i.includes("fagote") ||
          i.includes("trompa") ||
          i.includes("trompete") ||
  ...
  function handleSearch(text: string) {
    ...
    const baseList =
      category && category !== "all"
        ? musicas.filter((m) =>
            verificarCategoria(m, category.toString().toLowerCase()),
          )
        : musicas;
    ...
    if (text === "") {
      ...
      const filtrado = baseList.filter(
        (m) =>
  ...
  async function handleCreateMusic() {
    ...
    try {
      await acervoService.addMusica(
        { title: novoTitulo, arranger: novoArranjador },
        audioFile,
        pdfFiles,
      );
  ...
  const renderItem = ({ item }: { item: Musica }) => (
    ...
    >
      ...
      <View style={styles.cardContent}>
        ...
        <Text style={styles.cardArranger}>
        ...
        <View style={styles.badgesRow}>
          ...
          {item.partiturasPaths &&
            Object.keys(item.partiturasPaths).length > 0 && (
              <View style={styles.badge}>
                <FileText size={10} color="#666" />
                <Text style={styles.badgeText}>
                  {Object.keys(item.partiturasPaths).length} Parts.
                </Text>
              </View>
            )}
        </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            ...
            <ScrollView contentContainerStyle={styles.modalForm}>
              ...
              <TextInput
                style={styles.input}
                placeholder="Arranjador (Opcional)"
                placeholderTextColor="#666"
                value={novoArranjador}
                onChangeText={setNovoArranjador}
              />
